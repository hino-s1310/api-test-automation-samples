...
from .helpers.test_data import APIEndpoints, InvalidTestData, TestFileData
...
def test_upload_pdf_failure(test_client):
    ...
    response = test_client.post(
        APIEndpoints.UPLOAD, files={"file": (filename, content, content_type)}
    )
...
def test_list_files_failure(test_client):
    ...
    assert any(
        "greater than or equal to 1" in str(error.get("msg", ""))
    ...
    assert "detail" in data
    assert any(
        "less than or equal to 100" in str(error.get("msg", ""))
        for error in data["detail"]
    )
...
def test_cleanup_old_files_failure(test_client):
    ...
    assert "detail" in data
    assert any(
        "less than or equal to 365" in str(error.get("msg", ""))
        for error in data["detail"]
    )
