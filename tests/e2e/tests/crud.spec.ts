...
test.describe('CRUDテスト', () => {
  ...
  test('ファイルをアップロードする', async ({ request }) => {
    // ファイルをアップロードする
    const response = await uploadPdfFile(request, VALID_UPLOAD_DATA)
  ...
  test('ファイル詳細を取得する', async ({ request }) => {
    // 最初のテストでアップロードされたファイルのIDを取得
    const listResponse = await getFilesList(request)
    ...
    const responseBody = await response.json()
    expect(responseBody.id).toBeDefined()
    expect(responseBody.filename).toEqual(VALID_UPLOAD_DATA.filename)
    expect(responseBody.markdown).toContain('PDF変換結果')
    expect(responseBody.status).toEqual('completed')
  ...
  test('指定されたIDのファイルの変換ログを取得', async ({ request }) => {
    ...
    // ファイルの変換ログを取得する 
    const response = await getFileLogs(request, fileId)
    // レスポンスのステータスコードとメッセージを検証
    ...
    expect(responseBody.logs[0].file_id).toEqual(fileId)
  ...
  test('ファイルを再生成する', async ({ request }) => {
    ...
    // ファイルを再生成する
    const response = await reconvertFile(request, fileId)
    // レスポンスのステータスコードを検証
  ...
  test('ファイルを削除する', async ({ request }) => {
    ...
    // ファイルを削除する
    const response = await deleteFile(request, fileId)
    // レスポンスのステータスコードを検証
  ...
  test('ファイルが取得できないことを確認する', async ({ request }) => {
    // 新しいファイルをアップロードしてから削除
    const uploadResponse = await uploadPdfFile(request, VALID_UPLOAD_DATA)
    expect(uploadResponse.status()).toBe(200)
    
    const uploadData = await uploadResponse.json()
    const fileIdToDelete = uploadData.id

    // ファイルを削除
    const deleteResponse = await deleteFile(request, fileIdToDelete)
    expect(deleteResponse.status()).toBe(200)

    // 削除されたファイルIDでアクセスして404エラーを確認
    const response = await getFileDetail(request, fileIdToDelete)
    // レスポンスのステータスコードを検証
